...
export const GoogleGeminiEffect = ({
  ...
  const dots = Array.from({ length: isMobile ? 8 : 14 }, (_, i) => ({
    left: `${Math.random() * 90}%`,
    top: `${10 + Math.random() * 70}%`,
    width: isMobile ? 10 + Math.random() * 10 : 16 + Math.random() * 16,
    height: isMobile ? 10 + Math.random() * 10 : 16 + Math.random() * 16,
  ...
  // Adjust start/end offsets based on mobile - longer visible sections
  const leftStart = "0%";
  const leftEnd = isMobile ? "40%" : "45%"; // Longer left section on mobile
  const rightStart = isMobile ? "60%" : "55%"; // Longer right section on mobile
  const rightEnd = "100%";
  ...
  return (
    <div className={cn("sticky top-80 gemini-bg-fix", className)}>
      ...
      <div className="absolute left-1/2 -translate-x-1/2 bottom-0 w-[200px] md:w-[400px] h-[40px] md:h-[80px] bg-cyan-200/30 blur-2xl rounded-full z-0" />

      {/* Heading and subheading content */}
      <div className="relative z-10 max-w-4xl mx-auto px-4 text-center py-4 md:py-7">
        <motion.h1
          className="text-3xl sm:text-4xl md:text-7xl font-extrabold mb-2 md:mb-3 tracking-tight leading-tight"
        ...
        <motion.div
          ...
          className="min-h-16 md:min-h-20 flex items-center justify-center relative"
        >
          <motion.p
            ...
            className="text-sm sm:text-base md:text-2xl text-slate-600 dark:text-slate-400 px-2 font-medium relative"
            style={{
          ...
          <motion.div
            ...
            className="absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] w-16 md:w-24 bg-gradient-to-r from-transparent via-cyan-400/40 to-transparent"
          />
        </motion.div>
      </div>
      ...
      >
        ...
        {/* LEFT SIDE - Input words (Problems/Requirements) - Extended visible range */}
        {currentSet.input.map((word, index) => (
          ...
          >
            <textPath href={`#${pathIds[index]}`} startOffset={leftStart}>
              <animate
        ...
        {/* RIGHT SIDE - Output words (Solutions) - Extended visible range */}
        {currentSet.output.map((word, index) => (
          ...
          >
            <textPath href={`#${pathIds[index]}`} startOffset={rightStart}>
              <animate
                attributeName="startOffset"
                from={rightStart}
                to={rightEnd}
