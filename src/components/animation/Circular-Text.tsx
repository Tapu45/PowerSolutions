...
const CircularText: React.FC<CircularTextProps> = ({
  ...
  const logoRotate = useTransform(rotation, (r) => -r);
      ...
      >
        {logoSrc && (
          <motion.img
            src={logoSrc}
            alt={logoAlt}
            className="absolute inset-0 m-auto w-30 h-30 object-contain"
            style={{ rotate: logoRotate }}
          />
        )}
        {letters.map((letter, i) => {
          const rotationDeg = (360 / letters.length) * i;
          const factor = (Math.PI / letters.length) * 0.8; // reduced radius for tighter gap
          const x = factor * i;
          const y = factor * i;
